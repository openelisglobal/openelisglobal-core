...
function editSamplePopup(form) 
{
    ...
    //standardize request parameter naming
	var href = context+"/TestManagementEditSamplePopup.do?ID=0&accessionNumber="+myfield.value+"&"+sessionid;
	//alert("href "+ href);	
	...
	<tr>
		...
		<td valign="top">
			...
			<html:select name="<%=formName%>" property="oldAssignedTests" multiple="true" size="4" style="width: 200px">
                <%--bugzilla 2532--%>
				<app:optionsCollection name="<%=formName%>" property="tests" label="assignedSortedTestTreeDisplayValue" value="id" showDefault="false" />
			</html:select>
		</td>
	...
	<tr>		
		...
		<td>
            <html:button onclick="addTestPopup(window.document.forms[0]);" property="addTest" disabled="<%=Boolean.valueOf(addDisabled).booleanValue()%>">
    		  ...
    		  <%--bugzilla 2568--%>
    		  <logic:empty name="<%=formName%>" property="amendMode">
				<bean:message key="label.button.editTests" />
    		  </logic:empty>
			</html:button>
			&nbsp;
