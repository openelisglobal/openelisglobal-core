...
public class LabelMakerServlet extends HttpServlet implements IActionConstants {
	...
	private void prePrintLabels(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		for (String testId : testIdsArray) {
			tests.add(testDAO.getActiveTestById(Integer.parseInt(testId)));
		}
		...
		labelMaker.generatePrePrintLabels(Integer.parseInt(request.getParameter("numSetsOfLabels")),
				...
				Integer.parseInt(request.getParameter("numSpecimenLabelsPerSet")),
				request.getParameter("facilityName"), tests);
