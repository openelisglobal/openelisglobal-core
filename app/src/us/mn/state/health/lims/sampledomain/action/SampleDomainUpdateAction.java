 ...
 */
public class SampleDomainUpdateAction extends BaseAction {
	...
	protected ActionForward performAction(ActionMapping mapping,
		...
		PropertyUtils.copyProperties(sampleDomain, dynaForm);
		...
		try {
			...
			if (!isNew) {
				...
				sampleDomainDAO.updateData(sampleDomain);
			} else {
				// INSERT

				sampleDomainDAO.insertData(sampleDomain);
			}
			tx.commit();
		} catch (LIMSRuntimeException lre) {
    		//bugzilla 2154
			LogEvent.logError("SampleDomainUpdateAction","performAction()",lre.toString());	
			tx.rollback();
			...
			if (lre.getException() instanceof org.hibernate.StaleObjectStateException) {
				...
				if (lre.getException() instanceof LIMSDuplicateRecordException) {
					String messageKey = "sampledomain.sampledomain";
		...
		PropertyUtils.copyProperties(dynaForm, sampleDomain);
