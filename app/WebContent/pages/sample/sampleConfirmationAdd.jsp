...
<%
	...
	useMothersName = FormFields.getInstance().useField(
			Field.MothersName);
	useRequesterSiteList = FormFields.getInstance().useField(
			Field.RequesterSiteList);
...
"<%=StringUtil.getMessageForKey("label.button.remove")%>"
								+ "\" class=\"textButton\" onclick=\"removeRequesterTest( this, \'"
								+ sampleIndex + "\' );\" ></td>");
		if (clone.find("div")) {
			...
			clone.find("requesterSampleRowTwo").replace(protoMaxPattern,
					"input value=\"0\"");
		}
		...
		clone.html(clone.html()
				.replace(protoSampleIDPattern, "_" + sampleIndex));
		clone.html(clone.html().replace(protoFunction,
				"this, '" + sampleIndex + "')"));
		...
		if (selectPrototype) {
			$jq("#initialCondition_" + sampleIndex).asmSelect({
				removeLabel : "X"
			});
	...
	function removeRequestedTest(buttonElement, sampleIndex) {
		$("testRequestTable_" + sampleIndex).deleteRow(
				buttonElement.parentNode.parentNode.rowIndex);
	}
	...
	function getCheckBoxHtml(id, row, sampleIndex) {
		return "<input type='checkbox' id='select_" + row + "' value='" + id
				+ "' onclick='requestedTestChanged(" + sampleIndex + " );'>";
	}
	...
	function requestedTestChanged(sampleIndex) {
		...
		requestedTestsTable.style.border = somethingChecked ? "1px solid black"
				: "1px solid red";
		fieldValidator.setFieldValidity(somethingChecked, "requestedTests_"
				+ sampleIndex);
		setSaveButton();
	}
	...
	function addSamples() {
		...
		for ( var i = 0; i < sampleIndexs.length; ++i) {
			samplesXml += addSample("zero" == sampleIndexs[i].value ? "0"
					: sampleIndexs[i].value);
		}
		samplesXml += "</samples>";
		return samplesXml;
	}
	function /*string*/addSample(sampleIndex) {
		...
		sampleXml += "requesterSampleId='"
				+ $("requesterSampleId_" + sampleIndex).value + "' ";
		sampleXml += "sampleType='" + ($("sampleType_" + sampleIndex).value)
				+ "' ";
		sampleXml += "collectionDate='"
				+ ($("collectionDate_" + sampleIndex).value) + "' ";
<%if (FormFields.getInstance().useField(Field.CollectionTime)) {%>
	sampleXml += "collectionTime='"
				+ ($("interviewTime_" + sampleIndex).value) + "' ";
<%} else {%>
...
<%}%>
	...
	function /*string*/getRequestedTests(sampleIndex) {
		var requestList = "";
		var requestedTests = $("requestedTests_" + sampleIndex)
	...
	function /*string*/getNote(sampleIndex) {
		var singleQuote = /\'/g;
		var doubleQuote = /\"/g;
		return ($("note_" + sampleIndex).value.replace(singleQuote, "\\'")
				.replace(doubleQuote, '\\"'));
	}
</script>
<%
...
<html:hidden name='<%=formName%>' property="requestAsXML"
	styleId="xmlWad" />
<tiles:insert attribute="sampleConfirmationOrder" />
<hr />
<h3>
	<bean:message key="sample.entry.confirmation.test.request" />
</h3>
...
<div id=div_0>
	<table id="testRequestTable_0">
		<tr id="requesterSampleRowOne">
			<td><bean:message
					key="sample.entry.test.confirmation.requester.id" /></td>
			<td><input type="hidden" value="zero" class="sampleIndex" /> <input
				type="hidden" value="0" id="maxReferralTestIndex_0" /> <input
				type="text" id="requesterSampleId_0" onchange=" makeDirty();">
			</td>
			<td><bean:message key="sample.collectionDate" />&nbsp;<span
				style="font-size: xx-small;"><bean:message
						key="sample.date.format" /></span>:</td>
			<td><input type="text" id="collectionDate_0"
				name="collectionDate_0" maxlength="10"
				onkeyup="addDateSlashes(this,event);"
				onchange="checkValidDate(this)" /> <%
 	if (FormFields.getInstance().useField(Field.CollectionTime)) {
 %>
				<bean:message key="sample.collectionTime" />: <html:text
					name="<%=formName%>" onkeyup="filterTimeKeys(this, event);"
					property="interviewTime" styleId="interviewTime_0" maxlength="5"
					onblur="makeDirty(); updateFieldValidity(checkValidTimeEntry(this, true), this.id );" />
				<%
					}
				%></td>
			<td><img src="./images/note-add.gif"
				onclick="showHideNotes( '0');" id="showHideButton_0" /> <input
				type="hidden" name="hideShowFlag" value="hidden" id="hideShow_0">
			</td>
		</tr>
		<tr id="requesterSampleRowTwo">
			<%
				if (useInitialSampleCondition) {
			%>
			<td><bean:message key="sample.entry.sample.condition" /></td>
			<td><select id="initialCondition_0" multiple="multiple"
				title='<%=StringUtil.getMessageForKey("result.multiple_select")%>'>
					<logic:iterate id="optionValue" name='<%=formName%>'
						property="initialSampleConditionList" type="IdValuePair">
						<option value='<%=optionValue.getId()%>'>
			...
			%>
			<td><bean:message key="sample.entry.sample.type" />&nbsp;<span
				class="requiredlabel">*</span></td>
			<td><select id="sampleType_0"
				onchange="makeDirty(); populateRequestForSampleType(this, '0');">
					<option value="0"></option>
					<logic:iterate id="sampleTypes" name='<%=formName%>'
						property="sampleTypes" type="IdValuePair">
		...
		<tr id="noteRow_0" style="display: none;">
			<td style="vertical-align: top" align="right"><bean:message
					key="note.note" />:</td>
			<td colspan="6" align="left"><textarea id="note_0"
					onchange="makeDirty();" cols="100" rows="3"></textarea></td>
		</tr>
		<tr id="referralTestId_0">
			<td><bean:message
					key="sample.entry.test.confirmation.site.test" /></td>
			<td><select name="requesterTests" id="requestedTests_0_0"
				class="requestingTests_0"
				onchange="setPossibleResultsForTest(this, '0_0');">
					<option value="0">&nbsp;</option>
			</select></td>
			<td><bean:message
					key="sample.entry.test.confirmation.site.result" /></td>
			<td><select name="result" style="display: none"
				class="referralTestResult_0" id="dictionaryResult_0_0"
				onchange="checkDictionaryForUseText(this, '0_0');">
			</select> <input type="text" class="referralTestResult_0" name="result"
				style="display: none" id="textResult_0_0" /> <textarea rows="2"
					class="referralTestResult_0" name=result style="display: none"
					id="freeTextResult_0_0"></textarea></td>
		</tr>
		<tr id="addButtonRow">
			<td>&nbsp;</td>
			<td><input type="button" class=textButton
				value='<%=StringUtil
					.getMessageForKey("sampletracking.requester.test.result.add")%>'
				onclick="addNewRequesterTestResult(this, '0');" /></td>
		</tr>
		<tr id="requestedTestRow_0">
			<td style="vertical-align: top"><bean:message
					key="sample.entry.confirmation.requested.tests" /> : <span
				class="requiredlabel">*</span></td>
			<td colspan="5">
				<table
					style="background-color: #EEEEEE; color: black; border: 1px solid red"
					id="requestedTests_0">
				</table>
			</td>
		</tr>
...
<input type="button" class=textButton
	value="<%=StringUtil
					.getMessageForKey("sampletracking.requester.sample.add")%>"
	onclick="addNewRequesterSample( )" />
<hr />
<hr style="width: 100%; height: 5px" />
<html:hidden name="<%=formName%>" property="patientPK"
	styleId="patientPK" />
<table style="width: 100%">
	<tr>
		<td style="width: 15%; text-align: left;"><input type="button"
			value="+" onclick="showHideSamples(this, 'patientInfo');"> <bean:message
				key="sample.entry.patient" />: <%
 	if (patientRequired) {
 %><span
