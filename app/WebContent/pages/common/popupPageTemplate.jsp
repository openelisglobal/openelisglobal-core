...
function setAction(form, action, validate, parameters) {
    ...
    if (parameters != '' && (parameters == '?search=Y' || parameters == 'showall')) {
       if (parameters == '?search=Y')  
          parameters = parameters + "&accessionNumber=" + accessionNumber;
       else
	...
	//bugzilla 2503
    if (parameters != '' && parameters == '?search=Y')  {
  
       var fieldObj = window.document.forms[0].elements['filterString'];
       doFilter = 'Y';
       filterString = '';
       if (fieldObj != null) {
          filterString = fieldObj.value;
       }
    }
